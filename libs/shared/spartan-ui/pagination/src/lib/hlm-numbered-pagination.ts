...
export function createPageArray(
	...
	const isEllipsesNeeded = paginationRange < totalPages;
	let index = 1;

	while (index <= totalPages && index <= paginationRange) {
		let label: number | '...';
		const pageNumber = calculatePageNumber(index, currentPage, paginationRange, totalPages);
		const isOpeningEllipsesNeeded = index === 2 && (isMiddle || isEnd);
		const isClosingEllipsesNeeded = index === paginationRange - 1 && (isMiddle || isStart);
		label = isEllipsesNeeded && (isOpeningEllipsesNeeded || isClosingEllipsesNeeded) ? '...' : pageNumber;
		pages.push(label);
		index++;
	}
 ...
 * Copied from 'ngx-pagination' package
 */
function calculatePageNumber(index: number, currentPage: number, paginationRange: number, totalPages: number) {
	const halfWay = Math.ceil(paginationRange / 2);
