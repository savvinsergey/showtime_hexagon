...
export function classes(computed: () => ClassValue[] | string, options: ClassesOptions = {}) {
	runInInjectionContext(options.injector ?? inject(Injector), () => {
		const elementReference = options.elementRef ?? inject(ElementRef);
...
function restoreTransitionSuppression(manager: ElementClassManager): void {
	const previous = manager.previousTransition;
	if (previous) {
		manager.element.style.setProperty('transition', previous, manager.previousTransitionPriority || undefined);
	} else {
...
function updateElement(manager: ElementClassManager): void {
	...
	const sortedSources = [...manager.sources.entries()].sort(([, a], [, b]) => a.order - b.order);
